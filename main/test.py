...
def eval_accuracies(hypotheses, references):
    ...
    for k, v in hypotheses.items():
        if len(v) == 1:
            ...
            top_hypotheses[k] = v[:1]


    # Compute ROUGE scores
    rouge_calculator = Rouge()
    rouge_l, ind_rouge = rouge_calculator.compute_score(references, top_hypotheses)

    # Compute METEOR scores
    meteor_calculator = Meteor()
    meteor, _ = meteor_calculator.compute_score(references, top_hypotheses)
